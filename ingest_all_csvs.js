...
async function processFile(filePath) {
  return new Promise((resolve, reject) => {
    ...
    // Use the filename (without extension) as a fallback city name
    const fileNameCity = path.basename(filePath, '.csv').replace(/viator|\(\d+\)|classes/gi, '').trim() || 'Unknown';
    ...
    fs.createReadStream(filePath)
      ...
      .on('end', async () => {
        ...
        if (results.length > 0) {
            ...
            const citySlug = slugify(results[0].city);
            ...
            for (let i = 0; i < results.length; i += batchSize) {
              ...
              // Ensure city names match the actual inserted city name or just rely on text
...
async function run() {
  console.log("Starting MASS CSV ingestion into Supabase...");
